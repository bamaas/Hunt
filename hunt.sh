        ...
        raw_result=$(
          fzf "${fzf_opts[@]}" --ansi \
            ...
            --bind "result:transform-list-label:
                ...
                " \
            --bind "focus:change-input-label( Input )+transform-preview-label([ -n {1} ] && printf ' Previewing [%s] ' {1})" \
            --height=100% \
            --layout=default \
            --delimiter=: \
            --print-query \
        ...
        result=$(
          {
            ...
            --bind "focus:change-input-label( Input )+transform-preview-label([ -n {} ] && printf ' Previewing [%s] ' {})" \
            --height=100% \
            --layout=default \
            --no-sort --tac \
            --prompt="explore> " \
